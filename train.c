...
void trains_setup(void)
{
     ...
     */
	dcc_sender_t* dccobject = dcc_new(19,20, 63000);
...
int train_cmd(char* arg)
{
	 ...
	 */
	 memset(resp_msg,'\0',100);
	if (0 == strcmp(arg, "list"))
	{
		printf(
				"ID\tNAME\tPOWER\tTARGET\tDIRECTION\tSECTOR\tSECURITY\tACTIVE\n");
		int i;
		 snprintf(resp_msg,100,"ID NAME POWER TARGET DIRECTION SECTOR SECURITY ACTIVE<br>");
		for (i = 0; i < ntrains; ++i) {
		...
		memset(temp_msg,'\0',100);
		snprintf(temp_msg,100,"%d %s %d %d %s  %d %d %s <br>", trains[i]->ID,
                                        trains[i]->name, trains[i]->power, trains[i]->target_power,
                                        (trains[i]->direction) == FORWARD ? "FORWARD" : "REVERSE",
                                        trains[i]->telemetry->sector, trains[i]->security_override,
                                        (trains[i]->ID == current_train->ID) ? "<" : " ");
	...
	if (0 == strncmp(arg, "select ", strlen("select ")))
	{
		...
		for (i = 0; i < ntrains; i++) {
			if (trains[i]->ID == train_id) {
				...
				snprintf(resp_msg,100,"Selected train %d",train_id);
		                set_res(resp_msg,strlen(resp_msg));
				return 0;
			}
		}
		printf(
                ...
                snprintf(resp_msg,100,"Train not found. Use train list to see a list of available trains");
                set_res(resp_msg,strlen(resp_msg));
		return 1;
	}
	...
	if (0 == strncmp(arg, "speed ", strlen("speed ")))
	{
		...
		if (abs(target_speed) > 28) {
			printf("Speed must be between -28 and 28\n");
		 	snprintf(resp_msg,100,"Speed must be between -28 and 28");
        		set_res(resp_msg,strlen(resp_msg));
			return 1;
		} else {
			...
			snprintf(resp_msg,100,"Train %d %s speed set to %d\n", current_train->ID,
                                        current_train->name, target_speed);
                        set_res(resp_msg,strlen(resp_msg));
	...
	if (0 == strncmp(arg, "estop", strlen("estop")))
	{
		...
		snprintf(resp_msg,100,"Train %d %s stopped\n", current_train->ID, current_train->name);
                set_res(resp_msg,strlen(resp_msg));
